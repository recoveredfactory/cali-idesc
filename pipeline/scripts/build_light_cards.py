...
# halo strength). The streets ride ONE simple axis, and it runs like real street
# lights — SUNSET TO SUNRISE: glow-side (sodium-warm, LIGHTER than the ground)
# from dusk all the way through dawn, pavement-grey (DARKER than the ground)
# through the working day. No per-phase hue tricks — the point is VALUE contrast,
# and grey/yellow keep the grid clear of the blue water. The two polarity flips
# (dawn->morning, afternoon->dusk) land BETWEEN keyframes, mid-segment, where the
# eased cycle sweeps fastest — a card must beat the ground-mottling noise floor,
# and no tuned grey can do that AT a twilight card (alpha pulls any fill toward
# the ground; measured in probe_road_contrast.py). Opens on the moon, returns to her.
